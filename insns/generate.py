...
def header(f, insn):
    ...
    print("  input [`RISCV_FORMAL_XLEN   - 1 : 0] rvfi_mem_addr,", file=f)
    print("  input [`RISCV_FORMAL_XLEN/8 - 1 : 0] rvfi_mem_rmask,", file=f)
    print("  input [`RISCV_FORMAL_XLEN/8 - 1 : 0] rvfi_mem_wmask,", file=f)
    print("  input [`RISCV_FORMAL_XLEN   - 1 : 0] rvfi_mem_rdata,", file=f)
    print("  input [`RISCV_FORMAL_XLEN   - 1 : 0] rvfi_mem_wdata,", file=f)
...
def insn_l(insn, funct3, numbytes, signext):
    with open("insn_%s.v" % insn, "w") as f:
        ...
        print("  wire [%d-1:0] result = rvfi_mem_rdata >> (8*(addr-spec_mem_addr));" % (8*numbytes-1), file=f)
