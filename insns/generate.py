...
def insn_jal(insn = "jal"):
	with open("%s.vh" % insn, "w") as f:
		...
		format_uj(f)
		print("// %s instruction" % insn.upper(), file=f)
		print("wire [XLEN-1:0] result = pre_pc + insn_imm;", file=f)
		print("always @(posedge clk) begin", file=f)
		print("  if (valid && insn_opcode == 7'b 1101111) begin", file=f)
		print("    assert(rd == insn_rd);", file=f)
		print("`ifdef RISCV_FORMAL_COMPRESSED", file=f)
		print("    if (result[0] == 0) begin", file=f)
		print("`else", file=f)
		print("    if (result[1:0] == 0) begin", file=f)
		print("`endif", file=f)
...
def insn_jalr(insn = "jalr"):
	with open("%s.vh" % insn, "w") as f:
		...
		format_i(f)
		print("// %s instruction" % insn.upper(), file=f)
		print("wire [XLEN-1:0] result = pre_rs1 + insn_imm;", file=f)
		print("always @(posedge clk) begin", file=f)
...
def insn_b(insn, funct3, expr):
	with open("%s.vh" % insn, "w") as f:
		...
		print("wire [XLEN-1:0] result = (cond ? pre_pc + insn_imm : pre_pc + 4);", file=f)
		print("always @(posedge clk) begin", file=f)
		...
		print("    if (result[1:0] == 0) begin", file=f)
		print("`endif", file=f)
		print("      assert(post_pc == result);", file=f)
		print("      assert(!post_trap);", file=f)
		print("    end else begin", file=f)
		print("      assert(post_trap);", file=f)
		print("    end", file=f)
