...
// BLT instruction
wire cond = $signed(pre_rs1) < $signed(pre_rs2);
wire [XLEN-1:0] result = (cond ? pre_pc + insn_imm : pre_pc + 4);
always @(posedge clk) begin
  if (valid && insn_funct3 == 3'b 100 && insn_opcode == 7'b 1100011) begin
    assert(rs1 == insn_rs1);
    assert(rs2 == insn_rs2);
    assert(rd == 0);
